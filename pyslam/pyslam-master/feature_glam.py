...
class GLAMpoints2D:
    ...
    def pre_process_data(self, image1):
        '''
        ...
        image = self.center_crop(image1, (256, 256))
    ...
    def detect(self, frame, mask=None):
        ...
        output_nms = self.non_max_suppression(output.data.cpu().numpy().squeeze(), self.nms, self.min_prob)
        ...
        (kp,des) = self.feature.compute(frame, kp_cv2)
    ...
    def compute(self, frame, kps, eps=1e-7):
        ...
        kp_array = np.array([kp_map[1],kp_map[0]]).T
        kp_cv2 = [cv2.KeyPoint(kp_array[i,0], kp_array[i,1], 10) for i in range(len(kp_array))]
        (kps,des) = self.feature.compute(frame, kp_cv2)
